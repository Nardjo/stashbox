...
Domain glossary lives in [UBIQUITOUS_LANGUAGE.md](./UBIQUITOUS_LANGUAGE.md). This file captures _resolved domain decisions_ that don't fit a glossary entry — the "how it actually works" answers we've reached during interviews.
...
- **Fetch provider** is a first-class **Provider** alongside **LLM provider** and **Embedding provider**, swappable via env (`STASHBOX_FETCH_PROVIDER`).
...
2. **Host**:
   ...
   - apply hardcoded canonical aliases: **`twitter.com` → `x.com`**, **`youtu.be/<id>` → `x.com/watch?v=<id>`** _(see note)_, plus future entries as needed. The canonical side is the destination.
3. **Path**:
...
1. The **Owner** updates `STASHBOX_EMBEDDING_*` env vars.
...
If the **Owner** ever needs to manually correct a misclassified type (a Medium URL that actually serves a PDF, etc.), that is a post-v1 admin command (`stashbox set-type`) — not a **Refresh** behavior. **Refresh** replays the same deterministic rules.
...
- Filters are applied **before** the vector top-K, not after. The query becomes "find the K most similar **Bookmarks** _within the subset matching these filters_". This guarantees that asking for `--type article` always returns up to K articles (assuming enough exist), rather than returning fewer because the global top-K happened not to contain articles. Slightly slower than post-filtering but semantically what the **Owner** expects.
- Multi-tag matching is **OR by default** (`--tags ml,video` returns **Bookmarks** with _either_ tag), with **AND opt-in** via an explicit flag. OR fits the dominant single-user pattern of "broaden across angles"; AND is reserved for the rarer "precise intersection" use case.
...
The export is the **load-bearing promise** of stashbox ("what you save is yours"). It must let the **Owner** (a) walk away to another tool with as much data as that tool can absorb, and (b) restore a stashbox instance bit-for-bit without re-paying enrichment costs.

`stashbox export <target>` infers format from the target:

- `<file>.csv` — minimal portable CSV: `url, title, description, tags, type, enrichment_status, saved_at`. Same columns as the import format. Compatible with Linkding / Linkwarden / Pocket. **Embeddings and `embeddingSourceText` are NOT included.** Re-importing into stashbox triggers full **Enrichment**.
- `<file>.jsonl` — full-fidelity JSON Lines: every column of the **Bookmark** including `embedding`, `embeddingModel`, `embeddingSourceText`, `enrichmentFailureReason`, etc. Designed for round-trip restoration into a stashbox instance running the same **Embedding model**.
...
- `<directory>/` — writes the bundle: `bookmarks.csv` + `embeddings.jsonl` + `tags.csv`. Default for `stashbox export` without a target. Best of both worlds: portable CSV for migration, JSONL for lossless restore.

**Format versioning**: every export carries an explicit version marker.

- CSV: comment line `# stashbox-export v1` as first line (ignored by Excel / spreadsheet tools, read by `stashbox import`).
- JSONL: first line is a metadata record `{"_format": "stashbox-jsonl", "version": 1, "embeddingModel": "...", "embeddingDim": 1536, "exportedAt": "..."}`.
...
A **Bookmark** records the _set of distinct sources_ it has ever been saved from, alongside the existing counters.
...
Identification precedence at the API: an `X-Stashbox-Client` header takes priority; if absent, the **API key** name is mapped (e.g. a key named `shortcut-iphone` → `ios-shortcut`); if neither yields a known value, the source defaults to `api`.
...
React + TanStack Start interface for the **Owner** to browse, search, add, and delete **Bookmarks**.

**Design decisions:**

- Stack: React + TanStack Start, shadcn/ui + Tailwind CSS
- Rendering: SSR + Server Functions — all API calls go through server functions; `STASHBOX_API_URL` and `STASHBOX_API_KEY` env vars never reach the browser
- Layout: fixed grid of **Bookmark** cards (thumbnail via `ogImage`, title, tags, type badge)
- Add bookmark: inline card in the first grid position (URL input, submits via server function)
- Search: classic filter bar (top, filters by tag/type) + semantic AI search bar (uses `/search` endpoint)
