...
| **Save** | The act of submitting a URL to stashit for storage | Add, create, post |
...
| **Client** | A program that talks to the stashit API (Shortcut, Extension, CLI, MCP) | App, consumer, integration |
| **Agent** | An LLM-driven client (e.g. Claude Desktop) that consumes stashit via MCP | Bot, AI |
...
- **"Retry"**: Used in two distinct senses — (1) in-job pipeline attempts before a **Bookmark** becomes `failed`, and (2) explicit re-runs via `refresh_bookmark` / `stashit retry-failed`. Prefer **attempt** for (1) and **Refresh** for (2).
- **"Tag" the noun vs `tag` the CLI verb**: `stashit tag <tag>` is a *list-by-tag* command, not a tagging action. Document it as "list by **Tag**" — bookmarks are never manually tagged in v1; **Tags** are produced by **Enrichment**.
